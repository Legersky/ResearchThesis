...
% \end{minipage}}
\newcommand{\abstrEN}{We focus on the construction of algorithms for parallel addition in different numeration systems, especially the non-standard ones. The base $\beta$ is an element of $\Zomega$ and the alphabet, in general non-integer, is a subset of $\Zomega$ for some algebraic integer $\omega$. We design so-called extending window method with the basic rewriting rule $x-\beta$. The method searchs for a parallel addition algorithm for a given base $\beta$ and alphabet $\A$. It consists of two phases. We have the sufficient condition of the convergence of Phase 1. We introduce the algorithm which checks the necessary condition of convergence of Phase 2. The method is implemented in SageMath and we provide several tested examples.}
%%% k vyplnění KONEC ---------------------------------------------------
