...
The goal of the first phase is to compute a weight coefficient set $\Q$.   
    We start with $\{0\}$ meaning that we search all weight coefficients necessary for conversion for the case where there is no carry from the right. We add them to weight coeffcient set. These weight coefficient now may appear as a carry so we must check whether we have response -- weight coefficient for all combination of them and digits of input alphabet. And so on until there is no need to add more elements.
    
    Precise semi-algorithm is following: 
     $k:=0$
      ...
      \begin{itemize}
          \item extend $\Q_k$ to $\textcolor{red}{\Q_{k+1}}$ in a minimal possible way so that
           $$
              \B+ \Q_k \subset \A + \beta \textcolor{red}{\Q_{k+1}}\,,
           $$
           \item $k:=k+1$
      \end{itemize}
