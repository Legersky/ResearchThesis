...
$$
for all $j\in I_i$. If there is only one element in the index set $I_{d-1}=\{j_0\}$, choose element $x_{j_0}$. Otherwise choose $j_0\in I_{d-1}$ such that $x_{j_0,0}$ is the smallest from $x_{j,0}$ where $j\in I_{d-1}$. If there are more such elements, choose from them according to value $x_{j,1}$ etc. 

In other words, we take the elements which are ``closest'' to the rounded center of gravity $c$ of the values $x_1,\dots,x_n$ where ``closest''  is measured by absolute value of the first coordinate of $\pi(x_j)-\pi(c)$. In case of equality, according to the second coordinate etc. If there are more than one such element, we choose the element $x_{j_0}$ with the smallest first, resp. second, etc. coordinate of $\pi(x_{j_0})$.  

There is space to improve Phase 2 by a modification of Algorithm \ref{alg:minimalSet}. It is possible that the effort to reduce size of  $\Q_{[w_j,\dots, w_{j-m+1}]}$ as much as possible is not the best for convergence of Phase~2.
